...
export default function VoltagesChart({
  ...
  // --- Fechas X (con fallback para evitar 1969) ---
  const xDates = useMemo(() => {
    return data.length ? data.map((d) => new Date(d.ts)) : [new Date()];
  }, [data]);

  // --- Banda / Umbrales (solo visual; NO intervienen en autoscale) ---
  ...
  // --- Series (L1-N, L2-N, L3-N, Promedio) ---
    ...
    return [
      {
        ...
        hovertemplate: "%{y:.2f} V<extra>L1-N</extra>",
      },
      {
        x: safeX,
        ...
        hovertemplate: "%{y:.2f} V<extra>L2-N</extra>",
      },
      {
        x: safeX,
        ...
        hovertemplate: "%{y:.2f} V<extra>L3-N</extra>",
      },
      {
        x: safeX,
        ...
        hovertemplate: "%{y:.2f} V<extra>Promedio</extra>",
      },
    ];
  }, [xDates, data]);
  ...
  const baseLayout: Partial<Layout> = useMemo(
    () => ({
      uirevision: `voltages_v5_${yMin}_${yMax}_${followSec ?? "auto"}`,
      title: { text: "Voltajes (Tiempo Real)", font: { size: 16, color: colors.text } },
      autosize: true,
  ...
  const modal =
    ...
    createPortal(
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label="Gráfica de Voltajes - Ampliada"
      >
