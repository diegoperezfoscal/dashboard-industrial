// industrial-iot-lab/dashboard-industrial/src/components/VoltagesLLChart.tsx
"use client";
...
export const DEFAULT_LL_VOLTAGE_MIN = 380;
export const DEFAULT_LL_VOLTAGE_MAX = 440;
...
export default function VoltagesLLChart({
  data,
  height = 380,
  yRange,
  thresholds,
  ...
  // --- Umbrales ---
  const defaultMin = thresholds?.min ?? DEFAULT_LL_VOLTAGE_MIN;
  const defaultMax = thresholds?.max ?? DEFAULT_LL_VOLTAGE_MAX;
  ...
  const computeYRangeForVisibleX = useCallback(
    (startMs: number, endMs: number): [number, number] | null => {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        const v12 = Number.isFinite(row.L1_L2) ? (row.L1_L2 as number) : undefined;
        const v23 = Number.isFinite(row.L2_L3) ? (row.L2_L3 as number) : undefined;
        const v13 = Number.isFinite(row.L1_L3) ? (row.L1_L3 as number) : undefined;
        const vp = Number.isFinite(row.Promedio)
          ? (row.Promedio as number)
          : undefined;

        if (typeof v12 === "number") {
          if (v12 < minVal) minVal = v12;
          if (v12 > maxVal) maxVal = v12;
        }
        if (typeof v23 === "number") {
          if (v23 < minVal) minVal = v23;
          if (v23 > maxVal) maxVal = v23;
        }
        if (typeof v13 === "number") {
          if (v13 < minVal) minVal = v13;
          if (v13 > maxVal) maxVal = v13;
        }
        if (typeof vp === "number") {
  ...
  // === Reset a vista por defecto (1m + umbrales visibles) ===
  ...
  const traces: Data[] = useMemo((): Data[] => {
    ...
    return [
      {
        x: safeX,
        y: yOrNaN(data.map((d) => d.L1_L2 ?? NaN)),
        type: "scatter",
        mode: "lines",
        name: "L1-L2",
        showlegend: true,
        line: { color: "#1d4ed8", width: 1.8, shape: "spline", simplify: true },
        hovertemplate: "L1-L2: %{y:.2f} V<extra></extra>",
      },
      {
        x: safeX,
        y: yOrNaN(data.map((d) => d.L2_L3 ?? NaN)),
        type: "scatter",
        mode: "lines",
        name: "L2-L3",
        showlegend: true,
        line: { color: "#f59e0b", width: 1.8, shape: "spline", simplify: true },
        hovertemplate: "L2-L3: %{y:.2f} V<extra></extra>",
      },
      {
        x: safeX,
        y: yOrNaN(data.map((d) => d.L1_L3 ?? NaN)),
        type: "scatter",
        mode: "lines",
        name: "L1-L3",
        showlegend: true,
        line: { color: "#10b981", width: 1.8, shape: "spline", simplify: true },
        hovertemplate: "L1-L3: %{y:.2f} V<extra></extra>",
      },
      {
        x: safeX,
  ...
  const baseLayout: Partial<Layout> = useMemo(() => {
    return {
      uirevision: `voltagesLL_v1_${yMin}_${yMax}_fs${followSec ?? "auto"}_${
  ...
  const modal =
    ...
    createPortal(
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label="Tensiones L-L - Vista ampliada"
      >
        <div className="relative w-[min(1400px,95vw)] h-[min(90vh,900px)] rounded-xl border border-white/10 bg-[--third-paper] shadow-2xl">
          <div className="absolute left-0 right-0 top-0 h-12 flex items-center justify-between px-4 border-b border-white/10 bg-black/10 backdrop-blur">
            <div className="text-sm text-gray-100 font-medium">
              Tensiones L-L (Tiempo Real) — Vista ampliada
            </div>
            <div className="flex items-center gap-2">
              <button
                type="button"
